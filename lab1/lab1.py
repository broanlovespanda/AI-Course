...
# you need to.
sameidentity_rule = IF( OR('male (?x)', 'female (?x)'), THEN('same-identity (?x) (?x)'))
...
sibling_rule = IF( OR('brother (?x) (?y)', 'sister (?x) (?y)'), THEN('sibling (?x) (?y)'))
cousin_rule = IF( AND('parent (?z) (?x)', 'parent (?q) (?y)', 'sibling (?z) (?q)'), THEN('cousin (?x) (?y)'))
grandparent_rule = IF( AND('parent (?y) (?x)', 'parent (?z) (?y)'), THEN('grandparent (?z) (?x)', 'grandchild (?x) (?z)'))
# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [sameidentity_rule, father_rule, mother_rule, daughter_rule, son_rule, brother_rule, sister_rule, sibling_rule, cousin_rule, grandparent_rule ]                    # fill me in
